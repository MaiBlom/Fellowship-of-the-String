...
public class ForslagTilFileReader {
    private MediaDB db;
    private static ForslagTilFileReader instance;

    // This class is a singleton, because we will never need more than a single FileReader.
    private ForslagTilFileReader(MediaDB db){
        this.db = db;
    }
    public static ForslagTilFileReader getInstance(MediaDB db) {
        instance = new ForslagTilFileReader(db);
        return instance;
    }
